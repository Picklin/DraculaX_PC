...
bool Texture::loadFromMemory(const unsigned char* buffer, unsigned int length, PixelFormat format)
{
	unsigned char* image = NULL;
	...
	{
	case TEXTURE_PIXEL_FORMAT_RGB:
		image = SOIL_load_image_from_memory(buffer, length, &widthTex, &heightTex, 0, SOIL_LOAD_RGB);
		break;
	case TEXTURE_PIXEL_FORMAT_RGBA:
		image = SOIL_load_image_from_memory(buffer, length, &widthTex, &heightTex, 0, SOIL_LOAD_RGBA);
		break;
	}
